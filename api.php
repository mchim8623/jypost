...
function authenticateMonitor() {
    ...
    if (empty($token)) { http_response_code(401); die(json_encode(['error' => '缺少认证令牌'])); }
    global $db;
    $stmt = $db->prepare("SELECT id, name FROM monitors WHERE token = ? AND status = 1");
    $stmt->execute([$token]);
    $monitor = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$monitor) { http_response_code(401); die(json_encode(['error' => '无效的认证令牌'])); }
    return $monitor;
}
...
function sendWebhookAlert($server_id, $type, $message) {
    ...
    $stmt->execute(); $webhook = $stmt->fetchColumn();
    if (empty($webhook)) return;
    $stmt = $db->prepare("SELECT name FROM emby_servers WHERE id = ?"); $stmt->execute([$server_id]);
...
switch ($action) {
    ...
    case 'report':
        ...
        if (!isset($input['server_id']) || !isset($input['is_online'])) { http_response_code(400); die(json_encode(['error' => '缺少必要参数'])); }
        $stmt = $db->prepare("UPDATE monitors SET last_heartbeat = NOW(), ip = ? WHERE id = ?");
        $stmt->execute([$_SERVER['REMOTE_ADDR'], $monitor['id']]);
        ...
        if (!$input['is_online']) {
            $stmt = $db->prepare("SELECT is_online FROM monitor_data WHERE server_id = ? ORDER BY created_at DESC LIMIT 3");
            $stmt->execute([$input['server_id']]);
            $recent = $stmt->fetchAll(PDO::FETCH_COLUMN);
            $all_failed = (count($recent) >= 3 && array_sum($recent) == 0);
            if ($all_failed) {
                $stmt = $db->prepare("SELECT COUNT(*) FROM alert_logs WHERE server_id = ? AND alert_type = 'offline' AND created_at > DATE_SUB(NOW(), INTERVAL 30 MINUTE)");
                $stmt->execute([$input['server_id']]);

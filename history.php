...
$stats = [];
if ($server_id > 0 || $monitor_id > 0 || !empty($date)) { $stmt = $db->prepare("SELECT COUNT(*) as tc, SUM(CASE WHEN is_online=1 THEN 1 ELSE 0 END) as oc, AVG(response_time) as ar, MIN(response_time) as minr, MAX(response_time) as maxr FROM monitor_data md WHERE $whereSql"); $stmt->execute($params); $stats = $stmt->fetch(); }
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<div class="container-fluid"><div class="row">
    ...
    <main class="col-md-10 ms-sm-auto px-md-4 py-4">
        ...
        <?php if(!empty($stats)): ?>
        <div class="row mb-4">
            <div class="col-md-3"><div class="card stat-card"><div class="card-body"><h6>总检查</h6><h3><?= number_format($stats['tc']) ?></h3></div></div></div>
            <div class="col-md-3"><div class="card stat-card"><div class="card-body"><h6>在线率</h6><h3><?= $stats['tc']>0?round($stats['oc']/$stats['tc']*100,2):0 ?>%</h3></div></div></div>
            <div class="col-md-3"><div class="card stat-card"><div class="card-body"><h6>平均响应</h6><h3><?= round($stats['ar']??0) ?>ms</h3></div></div></div>
            <div class="col-md-3"><div class="card stat-card"><div class="card-body"><h6>响应范围</h6><h3><?= round($stats['minr']??0) ?>-<?= round($stats['maxr']??0) ?>ms</h3></div></div></div>
        </div>
        <?php endif; ?>

...
CREATE TABLE IF NOT EXISTS `emby_servers` (
  ...
  `use_custom_interval` tinyint(1) DEFAULT '0',
  `status` tinyint(1) DEFAULT '1',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
...
<div class="container"><div class="row justify-content-center"><div class="col-lg-8"><div class="install-card">
    <div class="install-header"><h2><i class="bi bi-stamp"></i> 集邮记</h2><p class="mb-0">Emby监控系统 - 安装向导</p></div>
    <div class="install-body">
        ...
        <table class="table"><thead><tr><th>检查项</th><th>要求</th><th>当前状态</th></tr></thead><tbody>
            <?php
            ...
            ?>
            <tr><td>PHP</td><td>≥7.4</td><td><span class="badge bg-<?= $php_ok?'success':'danger' ?>"><?= phpversion() ?></span></td></tr>
            <tr><td>PDO</td><td>已安装</td><td><span class="badge bg-<?= $pdo_ok?'success':'danger' ?>"><?= $pdo_ok?'已安装':'未安装' ?></span></td></tr>
            <tr><td>cURL</td><td>已安装</td><td><span class="badge bg-<?= $curl_ok?'success':'danger' ?>"><?= $curl_ok?'已安装':'未安装' ?></span></td></tr>
            <tr><td>JSON</td><td>已安装</td><td><span class="badge bg-<?= $json_ok?'success':'danger' ?>"><?= $json_ok?'已安装':'未安装' ?></span></td></tr>
            <tr><td>Mbstring</td><td>已安装</td><td><span class="badge bg-<?= $mbstring_ok?'success':'danger' ?>"><?= $mbstring_ok?'已安装':'未安装' ?></span></td></tr>
            <tr><td>OpenSSL</td><td>已安装</td><td><span class="badge bg-<?= $openssl_ok?'success':'danger' ?>"><?= $openssl_ok?'已安装':'未安装' ?></span></td></tr>
            <tr><td>写入权限</td><td>可写</td><td><span class="badge bg-<?= $writable_ok?'success':'danger' ?>"><?= $writable_ok?'可写':'不可写' ?></span></td></tr>
        </tbody></table>
        ...
        <form id="dbForm">
            ...
            <div class="mb-3"><label class="form-label">用户名</label><input type="text" class="form-control" name="db_user" value="root" required></div>
            <div class="mb-3"><label class="form-label">密码</label><input type="password" class="form-control" name="db_pass"></div>
        ...
        document.getElementById('testConnection').addEventListener('click',function(){
            var f=new FormData(document.getElementById('dbForm'));f.append('action','test_connection');
            document.getElementById('testResult').innerHTML='<span class="text-info">测试中...</span>';
            fetch('install.php',{method:'POST',body:f}).then(function(r){return r.json()}).then(function(d){
                document.getElementById('testResult').innerHTML=d.success?'<span class="text-success">✓ '+d.message+'</span>':'<span class="text-danger">✗ '+d.message+'</span>';
            });
        });
        document.getElementById('dbForm').addEventListener('submit',function(e){
            e.preventDefault();
            var p=this.querySelector('[name="admin_pass"]').value,c=this.querySelector('[name="admin_pass_confirm"]').value;
            if(p!==c){alert('两次密码不一致');return}
            if(p.length<6){alert('密码至少6位');return}
            ['db_host','db_port','db_name','db_user','db_pass','site_name','admin_user','site_url'].forEach(function(id){document.getElementById('install_'+id).value=this.querySelector('[name="'+id+'"]').value},this);
            document.getElementById('install_admin_pass').value=p;

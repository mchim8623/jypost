...
CREATE TABLE IF NOT EXISTS `monitors` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `name` varchar(100) NOT NULL COMMENT '监控机名称',
  `token` varchar(64) NOT NULL COMMENT '认证令牌',
  `ip` varchar(45) DEFAULT NULL COMMENT 'IP地址',
  `status` tinyint(1) DEFAULT '1' COMMENT '状态 0离线 1在线',
...
CREATE TABLE IF NOT EXISTS `emby_servers` (
  ...
  `url` varchar(255) NOT NULL COMMENT 'Emby服务器地址',
...
CREATE TABLE IF NOT EXISTS `monitor_data` (
  ...
  `monitor_id` int(11) NOT NULL COMMENT '监控机ID',
  ...
  `library_count` int(11) DEFAULT NULL COMMENT '媒体库数量',
  `library_details` text COMMENT '媒体库详情',
  `item_counts` text COMMENT '媒体项目计数(JSON)',
...
CREATE TABLE IF NOT EXISTS `alert_logs` (
  ...
  `alert_type` varchar(20) NOT NULL COMMENT 'offline/timeout',
            ...
            <div class="col-lg-8">
                <div class="install-card">
                    ...
                    <div class="install-body">
                        ...
                        <table class="table">
                            <thead>
                                <tr><th>检查项</th><th>要求</th><th>当前状态</th></tr>
                            </thead>
                            <tbody>
                                <?php
                                ...
                                $all_ok = $php_ok && $pdo_ok && $curl_ok && $json_ok && $mbstring_ok && $writable_ok;
                                ?>
                        ...
                        <div class="d-grid gap-2">
                            <?php if ($all_ok): ?>
                            <a href="?step=2" class="btn btn-primary btn-lg">下一步 <i class="bi bi-arrow-right"></i></a>
                            <?php else: ?>
                            <div class="alert alert-warning"><i class="bi bi-exclamation-triangle"></i> 请先满足所有环境要求后再继续安装</div>
                            <?php endif; ?>
                        </div>
                        ...
                        <form id="dbForm">
                            <div class="row mb-3">
                                <div class="col-md-8"><label class="form-label">数据库主机</label><input type="text" class="form-control" name="db_host" value="localhost" required></div>
                                <div class="col-md-4"><label class="form-label">端口</label><input type="text" class="form-control" name="db_port" value="3306" required></div>
                            </div>
                            ...
                            <div class="mb-3">
                                <button type="button" class="btn btn-outline-primary" id="testConnection"><i class="bi bi-database"></i> 测试连接</button>
                                <span id="testResult" class="ms-2"></span>
                            </div>
                            <hr>
                            <h5 class="mb-3">站点设置</h5>
                            ...
                            <div class="d-flex justify-content-between">
                                <a href="?step=1" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> 上一步</a>
                                <button type="submit" class="btn btn-primary">开始安装 <i class="bi bi-arrow-right"></i></button>
                            </div>
                        </form>
                        <form id="installForm" method="post" style="display:none;">
                            <input type="hidden" name="action" value="install">
                            <input type="hidden" name="db_host" id="install_db_host">
                            <input type="hidden" name="db_port" id="install_db_port">
                            <input type="hidden" name="db_name" id="install_db_name">
                            <input type="hidden" name="db_user" id="install_db_user">
                            <input type="hidden" name="db_pass" id="install_db_pass">
                            <input type="hidden" name="site_name" id="install_site_name">
                            <input type="hidden" name="admin_user" id="install_admin_user">
                            <input type="hidden" name="admin_pass" id="install_admin_pass">
                            <input type="hidden" name="site_url" id="install_site_url">
                        </form>
                        <script>
                        ...
                        document.getElementById('testConnection').addEventListener('click', function() {
                            ...
                            resultSpan.innerHTML = '<span class="text-info"><i class="bi bi-hourglass-split"></i> 正在测试...</span>';
                        ...
                        document.getElementById('dbForm').addEventListener('submit', function(e) {
                            e.preventDefault();
                            const pwd = this.querySelector('[name="admin_pass"]').value;
                            const confirm = this.querySelector('[name="admin_pass_confirm"]').value;

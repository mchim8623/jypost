...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) { $error = '请输入用户名和密码'; }
        ...
        if ($user && password_verify($password, $user['password'])) {
            $_SESSION['logged_in'] = true; $_SESSION['user_id'] = $user['id']; $_SESSION['username'] = $user['username']; $_SESSION['role'] = $user['role'];
...
<div class="container"><div class="row justify-content-center"><div class="col-md-5"><div class="login-card">
    ...
    <div class="login-body">
        ...
        <div class="text-center mt-3"><a href="public.php" class="text-muted"><i class="bi bi-eye"></i> 查看我的集邮成果</a></div>
    </div>
</div></div></div></div>
</body>
</html>
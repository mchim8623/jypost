...
<div class="container-fluid"><div class="row">
    ...
    <main class="col-md-10 ms-sm-auto px-md-4 py-4">
        ...
        <div class="alert alert-info"><i class="bi bi-info-circle"></i> 添加监控机后，将Token配置到Python程序。运行 <code>python monitor.py setup</code></div>
        <div class="card"><div class="card-body p-0"><table class="table table-hover mb-0"><thead><tr><th>ID</th><th>名称</th><th>IP</th><th>状态</th><th>最后心跳</th><th>1h检查</th><th>总检查</th><th>Token</th><th>操作</th></tr></thead><tbody>
            ...
            <tr><td><?= $m['id'] ?></td><td><?= htmlspecialchars($m['name']) ?></td><td><?= $m['ip'] ?? '-' ?></td><td><span class="status-badge <?= $online ? 'status-online' : 'status-offline' ?>"></span><?= $m['status'] ? ($online ? '在线' : '离线') : '已禁用' ?></td><td><?= $m['last_heartbeat'] ?? '-' ?></td><td><?= $m['c1h'] ?></td><td><?= $m['total'] ?></td><td><span class="token-display" title="<?= $m['token'] ?>"><?= substr($m['token'],0,8) ?>...</span><button class="btn btn-sm btn-outline-secondary" onclick="copyToken('<?= $m['token'] ?>')"><i class="bi bi-clipboard"></i></button></td><td><button class="btn btn-sm btn-outline-primary" onclick='editMonitor(<?= json_encode($m) ?>)'><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-outline-warning" onclick="regenerateToken(<?= $m['id'] ?>)"><i class="bi bi-arrow-repeat"></i></button><button class="btn btn-sm btn-outline-danger" onclick="deleteMonitor(<?= $m['id'] ?>)"><i class="bi bi-trash"></i></button></td></tr>
            <?php endforeach; ?>
        </tbody></table></div></div>
    </main>
</div></div>
...
<script>
    ...
    function editMonitor(m){document.getElementById('modalTitle').textContent='编辑监控机';document.getElementById('formAction').value='edit';document.getElementById('monitorId').value=m.id;document.getElementById('monitorName').value=m.name;document.getElementById('monitorStatus').value=m.status;document.getElementById('statusGroup').style.display='block';modal.show();}

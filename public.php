...
$publicServers = $db->query("
    SELECT es.id, es.name, es.url, es.icon_url,
        ...
        (SELECT COUNT(*) FROM monitor_data WHERE server_id=es.id ORDER BY created_at DESC LIMIT 3) as recent_count,
        (SELECT SUM(is_online) FROM (SELECT is_online FROM monitor_data WHERE server_id=es.id ORDER BY created_at DESC LIMIT 3) t) as recent_online
    FROM emby_servers es WHERE es.status=1 AND es.is_public=1
    ORDER BY CASE WHEN (SELECT item_counts FROM monitor_data WHERE server_id=es.id ORDER BY created_at DESC LIMIT 1) IS NOT NULL THEN CAST(JSON_EXTRACT((SELECT item_counts FROM monitor_data WHERE server_id=es.id ORDER BY created_at DESC LIMIT 1),'$.EpisodeCount') AS UNSIGNED) ELSE 0 END DESC, es.name ASC
")->fetchAll();

$publicStats = ['total_servers' => count($publicServers), 'online_servers' => count(array_filter($publicServers, fn($s) => $s['is_online'] == 1)), 'total_movies' => 0, 'total_series' => 0, 'total_episodes' => 0];
foreach ($publicServers as $s) { if ($s['item_counts']) { $c = json_decode($s['item_counts'], true) ?: []; $publicStats['total_movies'] += $c['MovieCount'] ?? 0; $publicStats['total_series'] += $c['SeriesCount'] ?? 0; $publicStats['total_episodes'] += $c['EpisodeCount'] ?? 0; } }

function calculateTrend($s) {
    $trend = ['movies' => ['change' => 0, 'direction' => 'flat'], 'series' => ['change' => 0, 'direction' => 'flat'], 'episodes' => ['change' => 0, 'direction' => 'flat'], 'total' => ['change' => 0, 'direction' => 'flat']];
    if (!$s['latest_counts'] || !$s['prev_counts']) return $trend;
    $l = json_decode($s['latest_counts'], true) ?: []; $p = json_decode($s['prev_counts'], true) ?: [];
    $types = ['MovieCount' => 'movies', 'SeriesCount' => 'series', 'EpisodeCount' => 'episodes']; $tl = $tp = 0;
    foreach ($types as $k => $n) { $lv = $l[$k] ?? 0; $pv = $p[$k] ?? 0; $ch = $lv - $pv; $trend[$n] = ['change' => $ch, 'direction' => $ch > 0 ? 'up' : ($ch < 0 ? 'down' : 'flat')]; $tl += $lv; $tp += $pv; }
    $tc = $tl - $tp; $trend['total'] = ['change' => $tc, 'direction' => $tc > 0 ? 'up' : ($tc < 0 ? 'down' : 'flat')];
    return $trend;
}
...
<div class="container">
    <div class="header">
        <div class="d-flex justify-content-between"><div><h1><i class="bi bi-stamp"></i> <?= htmlspecialchars($site_name) ?></h1><p class="text-muted mt-2 mb-0">Emby服务器实时监控状态</p></div><div class="text-end"><a href="login.php" class="admin-link"><i class="bi bi-lock"></i> 管理</a></div></div>
        <div class="stats-bar">
            ...
            <div class="stat-item ms-auto"><i class="bi bi-clock"></i><span class="stat-label"><?= $current_time ?></span></div>
        </div>
    </div>
    <?php if(empty($publicServers)): ?>
    <div class="empty-state"><i class="bi bi-server"></i><h4>暂无公开服务器</h4></div>
    <?php else: ?>
    <div class="server-grid">
        <?php foreach($publicServers as $s):
            $uptime = $s['total_checks_24h'] > 0 ? round($s['online_count_24h'] / $s['total_checks_24h'] * 100, 2) : 0;
            $lc = getLatencyClass($s['response_time']); $lt = getLatencyText($s['response_time']);
            $counts = $s['item_counts'] ? json_decode($s['item_counts'], true) : [];
            $trend = calculateTrend($s);
            $recentOnline = intval($s['recent_online'] ?? 0); $recentCount = intval($s['recent_count'] ?? 0);
        ?>
        <div class="server-card">
            ...
            <div class="server-body">
                <div class="metric-row"><span class="metric-label"><i class="bi bi-clock-history"></i> 响应延迟</span><span class="metric-value <?= $lc ?>"><span class="latency-badge <?= $lc ?>"><?= $lt ?> (<?= $s['response_time']??'-' ?>ms)</span></span></div>
                ...
                <div class="trend-section"><div class="trend-title"><i class="bi bi-graph-up-arrow"></i> 近期更新趋势</div><div class="trend-grid">
                    <?php if($trend['movies']['change']!=0): ?><div class="trend-item"><div class="label">电影</div><div class="value trend-<?= $trend['movies']['direction'] ?>"><?= $trend['movies']['change']>0?'+':'' ?><?= $trend['movies']['change'] ?></div></div><?php endif; ?>
                    <?php if($trend['episodes']['change']!=0): ?><div class="trend-item"><div class="label">单集</div><div class="value trend-<?= $trend['episodes']['direction'] ?>"><?= $trend['episodes']['change']>0?'+':'' ?><?= $trend['episodes']['change'] ?></div></div><?php endif; ?>
                    <div class="trend-item"><div class="label">总计</div><div class="value trend-<?= $trend['total']['direction'] ?>"><?= $trend['total']['change']>0?'+':'' ?><?= $trend['total']['change'] ?></div></div>
                </div></div>
                <?php endif; ?><?php endif; ?>
            </div>
...
<button class="refresh-btn" onclick="location.reload()"><i class="bi bi-arrow-clockwise"></i></button>
<script>let cd=30;setInterval(function(){cd--;if(cd<=0)location.reload()},1000);</script>
</body>
</html>
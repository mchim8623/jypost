...
$configs = []; $r = $db->query("SELECT * FROM system_config"); while ($row = $r->fetch()) $configs[$row['config_key']] = $row;
$stats = ['total_data' => $db->query("SELECT COUNT(*) FROM monitor_data")->fetchColumn(), 'oldest' => $db->query("SELECT MIN(created_at) FROM monitor_data")->fetchColumn(), 'alerts' => $db->query("SELECT COUNT(*) FROM alert_logs")->fetchColumn()];
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
